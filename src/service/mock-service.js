...
export default class MockService {
  ...
  generateOffers() {
    return TYPES.map((type) => ({
      type,
      offers: Array.from({lenght: getRandomIntenger(0, OFFER_COUNT)}, () => generateOffer(type))
    }));
  }
  ...
  generatePoints() {
    return Array.from({length: POINT_COUNT}, () => {
      ...
      const hasOffers = getRandomIntenger(0, 1);
      ...
      const offerIds = (hasOffers)
        ? offersByType.offers
          .slice(0, getRandomIntenger(0, OFFER_COUNT))
          .map((offer) => offer.id)
        : [];
